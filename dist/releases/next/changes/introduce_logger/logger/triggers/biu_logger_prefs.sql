...
-- changeset LOGGER:1772904356262 stripComments:false  logicalFilePath:introduce_logger\logger\triggers\biu_logger_prefs.sql
-- sqlcl_snapshot src/database/logger/triggers/biu_logger_prefs.sql:null:c750e9f4ac1e5486ea73dd339ab859f8e78d3ffe:create

create or replace editionable trigger logger.biu_logger_prefs before
    insert or update on logger.logger_prefs
    for each row
begin
  ...
  $else
    ...
    if
        ...
        and upper(:new.pref_value) not in ( logger.g_off_name, logger.g_permanent_name, logger.g_error_name, logger.g_warning_name, logger.g_information_name
        ,
                                            logger.g_debug_name, logger.g_timing_name, logger.g_sys_context_name, logger.g_apex_name )
    then
        raise_application_error(-20000, '"LEVEL" must be one of the following values: '
                                        ...
                                        || logger.g_debug_name
                                        || ', '
                                        || logger.g_timing_name
                                        || ', '
                                        || logger.g_sys_context_name
                                        || ', '
                                        || logger.g_apex_name);
    ...
    if
        1 = 1
        and :new.pref_type = logger.g_pref_type_logger
        and :new.pref_name not in ( 'GLOBAL_CONTEXT_NAME', 'INCLUDE_CALL_STACK', 'INSTALL_SCHEMA', 'LEVEL', 'LOGGER_DEBUG',
                                    'LOGGER_VERSION', 'PLUGIN_FN_ERROR', 'PREF_BY_CLIENT_ID_EXPIRE_HOURS', 'PROTECT_ADMIN_PROCS', 'PURGE_AFTER_DAYS'
                                    ,
                                    'PURGE_MIN_LEVEL' )
    then
